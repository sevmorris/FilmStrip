...
#
# FilmStrip only ever encodes audio (pcm_*, aac, opus, flac) and decodes video
# containers to reach it, so none of the GPL encoders the app used to bundle
# (x264, x265, libvidstab) are reachable from any code path. This build drops
# them, which removes the GPL Corresponding Source obligation entirely.
#
...
PBXPROJ="$PROJECT_DIR/FilmStrip.xcodeproj/project.pbxproj"
...
WORK="/tmp/filmstrip-ffmpeg-build"
rm -rf "$WORK"
mkdir -p "$WORK"
